...
[Latest Release / 最新版本](https://github.com/LexTheAries2209/Capricorn/releases/latest): `v2.7.4`

Bilingual release notes / 双语发布说明：[docs/releases/v2.7.4.md](docs/releases/v2.7.4.md)
...
1. 前往 [GitHub Releases](https://github.com/LexTheAries2209/Capricorn/releases/latest) 下载 `Capricorn-v2.7.4-macOS.zip`。
2. 解压后把 `Capricorn V2.7.4.app` 放到 `Applications` 或你的本地工具目录。
3. 首次打开时，如果 macOS Gatekeeper 提示来自互联网下载的 App，请在 Finder 中右键点击 App 后选择“打开”，或在“系统设置 > 隐私与安全性”中允许打开。

V2.7.4 的中文发布说明见 [docs/releases/v2.7.4.zh-CN.md](docs/releases/v2.7.4.zh-CN.md)。
...
- 测速页显示实时磁盘活动曲线；历史按每次测速任务分组，只保存和展示一张曲线，并把该次测速结果列在曲线下方。
...
- 设置页支持虚拟磁盘显示、序列号脱敏、内置 `smartctl` 版本与源项目入口、历史数据库位置打开、自动刷新、检查与修复菜单和 SMART 自检界面，并完整支持简体中文。序列号脱敏默认关闭；开启后只保留前四位，其余显示为 `*`，内部身份匹配仍使用完整值。
...
1. Download `Capricorn-v2.7.4-macOS.zip` from [GitHub Releases](https://github.com/LexTheAries2209/Capricorn/releases/latest).
2. Unzip it and move `Capricorn V2.7.4.app` to `Applications` or your local tools folder.
...
English release notes for V2.7.4 are available at [docs/releases/v2.7.4.en.md](docs/releases/v2.7.4.en.md).
...
- Benchmark History groups each run with one saved activity chart above the results recorded by that run.
