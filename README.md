...
[Latest Release / 最新版本](https://github.com/LexTheAries2209/Capricorn/releases/latest): `v2.9.1`

Bilingual release notes / 双语发布说明：[docs/releases/v2.9.1.md](docs/releases/v2.9.1.md)
...
1. 前往 [GitHub Releases](https://github.com/LexTheAries2209/Capricorn/releases/latest) 下载 `Capricorn-v2.9.1-macOS.zip`。
2. 解压后把 `Capricorn V2.9.1.app` 放到 `Applications` 或你的本地工具目录。
3. 首次打开时，如果 macOS Gatekeeper 提示来自互联网下载的 App，请在 Finder 中右键点击 App 后选择“打开”，或在“系统设置 > 隐私与安全性”中允许打开。

V2.9.1 的中文发布说明见 [docs/releases/v2.9.1.zh-CN.md](docs/releases/v2.9.1.zh-CN.md)。
...
- 保存 SMART、测速和实时活动历史，按需要隐藏或恢复记录，并可在设置中开启逐条永久删除。
...
- 当三星 Portable SSD T5 在 SAT SMART Drive 已安装后仍无 SMART 数据时，概览和 SMART 页面会提示三星驱动可能存在冲突；识别到 USB-NVMe 且 SAT 仍无法取得 SMART 时，会说明部分 USB-NVMe 桥接器在 macOS 上可能无法提供 SMART 数据。
- 支持默认、峰值/NVMe、真实场景、演示和自定义测速配置。
- 测速配置可以选择仅读取、仅写入或读写，按需要只执行对应测试项目。
...
- 测速目标跟随侧边栏所选硬盘及其代表卷，并支持选择同盘分区或文件夹；启动前会再次校验目标归属。重命名与实时活动的大文件自动负载同样使用代表卷。
- 测速页显示实时磁盘活动曲线；历史按每次测速任务分组，只保存和展示一张曲线，并把该次测速结果列在曲线下方。隐藏、恢复和删除以整组测速任务为单位处理，紧凑图表会跳过部分速度标签以保持刻度线对齐。
- 实时活动页支持 0.1s、0.2s、0.5s、1s 采样间隔、图表保留和保存到历史；已保存记录会在历史页显示对应折线图。
...
- 历史页支持 SMART、测速、实时活动记录的隐藏、管理隐藏记录和恢复；设置中另有默认关闭的逐条删除功能，开启后会在二次确认后永久删除可见或已隐藏记录。记录优先按完整硬件序列号匹配，无可用序列号时使用卷 UUID 兜底。V2.0.1 使用新的 `CapricornHistory` 数据库，不兼容旧版历史记录。
- 历史页的旧 JSON、CSV 和纯文本复制功能已移除，等待新的统一导出设计。
- SMART 历史摘要显示健康、寿命、介质错误、累计读写量和通电时间；每条快照可在独立滚动窗口中查看完整 SMART 报告，并可选择文件夹后再次导出 CSV。
- SMART 快照导出为紧凑 CSV，在同一属性表中加入磁盘身份、设备类型、卷 UUID 和外接盘卷信息；中文界面额外包含中文名称，英文界面使用 SMART 属性的英文名称和说明。
- SMART 页面分别提供“保存到历史”和“保存 CSV”；`Command-S` 保存 CSV 的同时仍会把当前快照写入历史。
...
- 设置页支持虚拟磁盘显示、序列号脱敏、内置 `smartctl` 版本与源项目入口、历史数据库位置打开、历史逐条删除开关、自动刷新、检查与修复菜单和 SMART 自检界面，并完整支持简体中文。序列号脱敏和历史删除默认关闭；序列号脱敏开启后只保留前四位，其余显示为 `*`，内部身份匹配仍使用完整值。
- 应用菜单只保留系统 `Settings…` 设置入口，并显示反引号/波浪号键快捷键；物理按键在不同输入法下均可打开设置。另支持 `Command-R` 刷新，`Tab` 始终按顺序切换功能页。
...
1. Download `Capricorn-v2.9.1-macOS.zip` from [GitHub Releases](https://github.com/LexTheAries2209/Capricorn/releases/latest).
2. Unzip it and move `Capricorn V2.9.1.app` to `Applications` or your local tools folder.
...
English release notes for V2.9.1 are available at [docs/releases/v2.9.1.en.md](docs/releases/v2.9.1.en.md).
...
- Save SMART, benchmark, and live activity history with hide and restore controls, plus an optional per-record permanent delete mode.
...
- If a Samsung Portable SSD T5 still lacks SMART data after SAT SMART Drive is installed, Overview and SMART warn about a possible Samsung-driver conflict. If a USB-NVMe device still cannot return SMART through SAT, Capricorn explains that some USB-NVMe bridges cannot expose SMART data on macOS.
- Includes Default, Peak/NVMe, RealWorld, Demo, and Custom benchmark profiles.
- Benchmark configurations can run read-only, write-only, or read-and-write test selections.
...
- Benchmark targets follow the drive selected in the sidebar and its representative volume, while still allowing a partition or same-drive folder override. Ownership is validated again before a run starts. Renaming and Live Activity automatic large-file workloads use the same representative volume.
- Benchmark History groups each run with one saved activity chart above the results recorded by that run. Hide, restore, and delete actions treat the run as one unit, while compact charts skip selected speed labels to keep grid lines aligned.
- Live Activity supports 0.1s, 0.2s, 0.5s, and 1s sampling, retained charts, and history saving; saved records show their chart in History.
...
- History supports SMART, benchmark, and activity record hiding, hidden-record management, and restore. Settings also provides a default-off individual delete mode; when enabled, visible or hidden records are permanently deleted after confirmation. Records match by full hardware serial first and fall back to volume UUIDs when a serial is unavailable. V2.0.1 uses a new `CapricornHistory` database and does not import legacy history.
- The legacy JSON, CSV, and plain-text copy controls were removed from History pending a redesigned unified exporter.
- SMART History summaries show health, life remaining, media errors, cumulative reads and writes, and power-on time. Every snapshot can open a separate scrolling full-report window and export another CSV after choosing a folder.
...
- The SMART page provides separate Save to History and Save CSV actions. `Command-S` saves the CSV and still adds the current snapshot to History.
...
- Settings cover virtual-drive visibility, serial-number redaction, the bundled `smartctl` version and smartmontools project link, history-database reveal, the individual history delete switch, automatic refresh, the Check and Repair menu, and the SMART self-test interface, with complete Simplified Chinese content. Redaction and history deletion are off by default; when redaction is enabled, only the first four characters remain visible while internal identity matching still uses the complete serial.
- The application menu keeps only the system `Settings…` command and shows the grave/tilde key shortcut. The physical key opens Settings across input sources. Capricorn also supports `Command-R` to refresh, while `Tab` always advances through feature pages in order.
