...
struct ExternalSupportView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                Button {
                    ...
                    Label(
                        language.t("External Drive SMART"),
                ...
                .buttonStyle(.plain)
                .help(language.t(isExpanded ? "Collapse External SMART" : "Expand External SMART"))
                Spacer()
                verificationBadge
                Button {
                    refresh()
    ...
    private var verificationBadge: some View {
        if isVerified {
            Label(language.t("Verified"), systemImage: "checkmark.seal.fill")
                .foregroundStyle(.green)
                .font(.callout.weight(.semibold))
        } else if diagnostics?.openError?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false {
            Label(language.t("Needs Attention"), systemImage: "exclamationmark.triangle.fill")
                .foregroundStyle(.orange)
                .font(.callout.weight(.semibold))
        } else {
            Label(language.t("Not Verified"), systemImage: "questionmark.circle")
                .foregroundStyle(.secondary)
                .font(.callout.weight(.semibold))
        }
    }
